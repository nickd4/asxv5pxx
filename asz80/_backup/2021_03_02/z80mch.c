...
struct mne *mp;
{
	...
	op = (int) mp->m_valu;
	rf = mp->m_type;
	if (((mchtyp == X_Z80) && (rf > S_CPU)) ||
	    ((mchtyp == X_8080) && (rf > S_CPU)))
		xerr('o', "Not a Z80 or 8080 instruction.");

	switch (rf) {
	case S_INH1:
		if ((mchtyp == X_8080) && (op == 0xD9))	/* EXX */
			xerr('a', "8080: No EXX instruction.");
		outab(op);
		break;

	case S_INH2:
		if (mchtyp == X_8080)
			xerr('a', "Not an 8080 instruction.");
		outab(0xED);
		outab(op);
		break;
